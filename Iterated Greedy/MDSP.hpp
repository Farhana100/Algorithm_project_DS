...
public:
     ...
     */
    vector<int> getDominatedCount(const vector<int> &D) {
     ...
     */
    vector<int> checkP(const vector<int> &D) {
     ...
     */
    int GIP(int u, const vector<int> &D) {
     ...
     * @return vector<int> 
     */
    vector<int> LocalImprovement(vector<int> D) {
        vector<int> D_b;
        vector<int> dominatedCount = getDominatedCount(D, G);
        vector<int> notD;
     ...
     */
    vector<int> Reconstruction(vector<int> D) {
        while(true) {                               // O(|D|) = O(n/2) = O(n)
            int maxGIP = 0;
            int toPush = -1;
            for(int u = 0; u < n; u++) {            // O(n)
     ...
     */
    vector<int> IteratedGreedy(int beta, int delta) {
